...
バックエンドを軸に、インフラ構築からフロントエンドまでフルスタックで開発を担当。マルチテナント型ワークフォースマネジメント SaaS では、AWS KMS によるテナント別暗号化基盤や DDD / CQRS に基づくドメインモデル設計、Server Driven UI アーキテクチャの導入、e-Gov 電子申請システムの認証連携からエンドツーエンドフロー構築、AWS Step Functions / Lambda / Batch によるサーバーレス ETL プラットフォームの設計・構築、AST 解析による約 600 ファイルの大規模コード変換（50 時間→6 時間に短縮）など、セキュリティ・アーキテクチャ・インフラを横断した設計・実装を単独で推進。前職ではレガシーシステムのリプレイスで認証基盤をはじめ複数機能の設計・実装を一貫して担当し、障害ゼロで安定した移行を実現。TiDB 移行では約 300 万件のデータに対する複数の障害を調査・復旧し完遂。チーム横断での運用ルール策定、技術広報活動（外部登壇 6 回）など、技術面だけでなく組織改善にも幅広く取り組む。
...
- **アーキテクチャ設計**: DDD / CQRS によるドメインモデル設計、Server Driven UI、手続き追加に対応するプラガブルなアーキテクチャ、サーバーレス ETL パイプライン設計
...
- **大規模コード変換・DX 改善**: AST 解析による約 600 ファイルの自動変換（50 時間→6 時間に短縮）、npm パッケージによる定義一元管理
