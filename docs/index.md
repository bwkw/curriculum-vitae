...
バックエンドを軸に、インフラ構築からフロントエンドまでフルスタックで開発を担当。現職のマルチテナント型ワークフォースマネジメント SaaS では、AWS KMS によるテナント別暗号化基盤、DDD / CQRS に基づくドメインモデル設計、Server Driven UI アーキテクチャの導入、e-Gov 電子申請システムの認証連携からエンドツーエンドフローの構築、AWS Step Functions / Lambda / Batch によるサーバーレス ETL プラットフォームの設計・構築、AST 解析による約 600 ファイルの大規模コード変換（50 時間→6 時間に短縮）などを推進。前職ではレガシーシステムのリプレイスで認証基盤をはじめ複数機能の設計・実装を一貫して担当し、障害ゼロで安定した移行を実現。また、レバテックでもっとも歴史があり最大容量の DB の Amazon RDS から TiDB への移行プロジェクトにおいて、移行後に発生した約 300 万件規模のデータ障害の調査・復旧と、移行知見のドキュメント整備と全チームへの展開を担当。チーム横断での運用ルール策定、技術広報活動（外部登壇 6 回）など、技術面だけでなく組織改善にも幅広く取り組む。
...
- **アーキテクチャ設計**: DDD / CQRS によるドメインモデル設計、Server Driven UI、プラガブルなアーキテクチャ、サーバーレス ETL パイプライン設計
- **バックエンド開発**: NestJS / Prisma / PostgreSQL での API 設計・実装、認証基盤（OIDC / JWT / OAuth 2.0 + PKCE）、外部 API 連携
- **セキュリティ基盤**: テナント別暗号化、監査ログ・MFA 強制、法令準拠のデータ管理設計
- **インフラ構築・運用**: AWS CDK / Terraform による IaC、Step Functions / Lambda / DynamoDB / Batch によるサーバーレス基盤構築
- **大規模コード変換・DX 改善**: AST 解析による自動変換ツール開発、npm パッケージによる定義一元管理、CI/CD 最適化
- **システム移行・リプレイス**: 認証基盤・データベース・業務機能のリプレイス設計・実装、安全なデータ移行
- **技術広報・育成**: 外部登壇、技術ブログ執筆、未経験者の登壇サポート、勉強会の設計・実施
...
| 日付       | イベント                                                                                     | タイトル                                                                                                                                |
| ---------- | -------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------- |
| 2026/01/31 | [SRE Kaigi 2026](https://2026.srekaigi.net/)                                                 | [制約が導く迷わない設計 〜 信頼性と運用性を両立するマイナンバー管理システムの実践 〜](https://speakerdeck.com/bwkw/zhi-yue-gadao-kumi-wanaishe-ji-xin-lai-xing-toyun-yong-xing-woliang-li-surumainanbaguan-li-sisutemunoshi-jian) |
...
gantt
    ...
    詳細画面の Server Driven UI 設計・導入                         :2025-11, 2026-01
...
- 参考：[制約が導く迷わない設計 〜 信頼性と運用性を両立するマイナンバー管理システムの実践 〜](https://speakerdeck.com/bwkw/zhi-yue-gadao-kumi-wanaishe-ji-xin-lai-xing-toyun-yong-xing-woliang-li-surumainanbaguan-li-sisutemunoshi-jian)（SRE Kaigi 2026 登壇）
...
- プロセス改善と品質管理
  ...
  - 取り組みと成果
    ...
    - モブプログラミングを導入し、役割分担・交代制・振り返りのルールを整備して属人化を解消
...
- 運用改善
  ...
  - 取り組みと成果
    - ログ設計指針を策定し、AsyncLocalStorage によるリクエストスコープのコンテキスト保持を導入。エラー発生ユーザの迅速な特定を可能にした
    - 監視ダッシュボードを 0→1 で構築し、アラート設定と定期的なメトリクスレビューの体制を整備。システムの安定稼働を実現
...
- 不要コード削除の推進
  ...
  - 取り組みと成果
    - CloudWatch Logs Insight で正規表現によりアクセスパスを抽出し、ルーティング定義と照合して削除対象を特定。内部処理の見落とし防止のため検証用ログを埋め込み、安全性を担保
...
- 認証・認可基盤のリプレイス
  ...
  - 取り組みと成果
    - OIDC と認可制御（RBAC / PBAC）の勉強会を設計・実施し、チーム全体の共通認識を形成（参考：[OIDC を Hono × Bun で完全に理解する](https://zenn.dev/levtech/articles/what-is-oidc)）
...
- インフラ構築
  ...
  - 取り組みと成果
    ...
    - VPC ルーティング、セキュリティグループ、ターゲットグループ、コンテナポートを順に検証し、ターゲットグループの転送先ポート設定の誤りを特定・解消

#### TiDB 移行プロジェクト（2024/05〜2024/06）

##### プロジェクト概要

- レバテックでもっとも歴史があり最大容量の DB の Amazon RDS から TiDB への移行プロジェクトに参画。移行作業は担当せず、移行後に発生した障害の調査・復旧と、移行知見のドキュメント整備と全チームへの展開を担当
...
- Python、Shell Script、MySQL、TiDB
...
- 移行後の障害対応と復旧、後続移行への知見共有
  - 課題・問題点
    - 移行完了後、TiDB 固有の採番方式の違い、タイムゾーンの不整合、データ型の制約差異、文字コードの Collation 変更など、複数の障害が発生した
  - 取り組みと成果
    - 各障害の原因を体系的に調査し、約 300 万件のデータに対するリペアスクリプトの作成・実行により全件復旧を完遂
    - 移行時の知見を社内 LT で発表し、CTO・SRE チームと連携して TiDB 移行チェックリストのドキュメントを整備し、全チームへ展開。後続移行で同様の障害が起きないよう予防に貢献
...
- 業務自動化と効率化
  ...
  - 取り組みと成果
    - BigQuery と Google Sheets を連携した自動集計の仕組みを 4 件構築し、手作業によるミスのリスクを排除
