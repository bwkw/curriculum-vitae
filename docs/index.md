...
- データ移行時の障害対応と復旧
  - 課題・問題点
    - TiDB 固有の採番方式の違い、タイムゾーンの不整合、データ型の制約差異、文字コードの Collation 変更など、移行に伴う複数の障害が発生した
  - 取り組みと成果
    - 各障害の原因を体系的に調査し、約 300 万件のデータに対するリペアスクリプトの作成・実行により全件復旧を完遂
    - 移行時の知見を社内 LT で発表し、CTO・SRE チームと連携して TiDB 移行チェックシートを作成。他チームの移行時の障害予防に貢献
