...
- 組織への知見共有と再発防止
  ...
  - 打ち手・使用した技術
    ...
    - 自チームで TiDB のローカル環境を導入した際、他チームでも導入が進むことを見越して、1 コマンドで起動できるスクリプトを作成し、全体に共有しました
...
- 重複 CV 改善施策の設計・実装
  ...
  - 課題・問題点
    ...
    - 複数システム（認証基盤、外部サイト）との連携が必要で、システム間の接点設計が必要でした
  - 打ち手・使用した技術
    ...
    - 認証基盤チームや外部サイト連携チームとの接点を整理し、EventBridge デプロイ時のパラメータ連携を事前に依頼することで、デプロイと動作確認をスムーズに実施しました
...
- 業務自動化と効率化
  ...
  - 打ち手・使用した技術
    ...
    - 公開中案件の評判表示率のデータ手動集計による将来的な作業ミスのリスクを回避しつつ、運用負荷の軽減を図るため、BigQuery と SS 間でデータ自動集計の仕組みを作成しました。ディレクターと自動化の実現方法の検討、データ集計タイミングや運用方法の調整、自動化のためのスクリプト作成を実施しました
